-- ============================================================
-- 020_security_hardening.sql (Final Hardened Version)
-- ============================================================

-- 1. HELPERS FOR SECURE ROLE CHECKS (Pinned Search Path)
...
-- 4. POLICIES FOR SECURED TABLES (Idempotent)
DROP POLICY IF EXISTS "Public read access for active schemes" ON wallet_schemes;
CREATE POLICY "Public read access for active schemes" ON wallet_schemes FOR SELECT USING (active = true);

DROP POLICY IF EXISTS "Admins can manage schemes" ON wallet_schemes;
CREATE POLICY "Admins can manage schemes" ON wallet_schemes FOR ALL TO authenticated USING (public.is_admin());

DROP POLICY IF EXISTS "Agents view own wallets" ON agent_wallet_accounts;
CREATE POLICY "Agents view own wallets" ON agent_wallet_accounts FOR SELECT USING ((SELECT agent_id FROM public.users WHERE id = auth.uid()) = agent_id);

DROP POLICY IF EXISTS "Supervisors view network wallets" ON agent_wallet_accounts;
CREATE POLICY "Supervisors view network wallets" ON agent_wallet_accounts FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.agents WHERE agents.id = agent_wallet_accounts.agent_id AND agents.supervisor_id = (SELECT supervisor_id FROM public.users WHERE id = auth.uid()))
);

DROP POLICY IF EXISTS "Admins view all wallets" ON agent_wallet_accounts;
CREATE POLICY "Admins view all wallets" ON agent_wallet_accounts FOR SELECT USING (public.is_admin());

DROP POLICY IF EXISTS "Supervisors view own commissions" ON commissions;
CREATE POLICY "Supervisors view own commissions" ON commissions FOR SELECT USING (supervisor_id = (SELECT supervisor_id FROM public.users WHERE id = auth.uid()));

DROP POLICY IF EXISTS "Admins view all commissions" ON commissions;
CREATE POLICY "Admins view all commissions" ON commissions FOR SELECT USING (public.is_admin());
...
-- 5. REPLACING METADATA ROLES (Security Fixes)
...
CREATE POLICY "Users view targeted banners" ON system_banners FOR SELECT USING (
    ...
    AND (target_role IN ('ALL', public.get_auth_role())
      OR (target_role = 'MY_AGENTS' AND EXISTS (SELECT 1 FROM public.users WHERE users.id = auth.uid() AND users.supervisor_id = system_banners.created_by)))
);

CREATE POLICY "Admin manage banners" ON system_banners FOR ALL USING (public.is_admin());

DROP POLICY IF EXISTS "Supervisors view self" ON supervisors;
CREATE POLICY "Supervisors view self" ON supervisors FOR SELECT USING (id = (SELECT supervisor_id FROM public.users WHERE id = auth.uid()) OR public.is_admin());

DROP POLICY IF EXISTS "v1_agents_read" ON agents;
DROP POLICY IF EXISTS "Secure agents access" ON agents;
CREATE POLICY "Secure agents access" ON agents FOR SELECT USING (id = (SELECT agent_id FROM public.users WHERE id = auth.uid()) OR supervisor_id = (SELECT supervisor_id FROM public.users WHERE id = auth.uid()) OR public.is_admin());

DROP POLICY IF EXISTS "v1_users_network" ON users;
DROP POLICY IF EXISTS "Secure users access" ON users;
CREATE POLICY "Secure users access" ON users FOR SELECT USING (id = auth.uid() OR supervisor_id = (SELECT supervisor_id FROM public.users WHERE id = auth.uid()) OR public.is_admin());
